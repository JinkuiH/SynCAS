...
#         """
#         SoftDiceLoss with sample weighting.
#         """
...
#         """
#         pred: predicted logits, shape (n, 1, x, y, z)
...
#         weight = weight.view(-1, 1, 1, 1, 1)  # Reshape to broadcast
...
#         # dice_loss = 1 - dice_score.mean()  # Average across batch
#         dice_score = dice_score[:, 1:]
#         dice_loss = 1 - (dice_score * weight.view(-1)).sum() / weight.sum()

#         return dice_loss
    
class SoftDiceLossWeight(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, clip_tp: float = None):
        """
        """
        super(SoftDiceLossWeight, self).__init__()
    ...
    def forward(self, x, y, weight,loss_mask=None):
        ...
        if not self.do_bg:
            if self.batch_dice:
                ...
                dc = dc[:, 1:]
        # dc = dc.mean()
        dc = (dc.view(-1) * weight).sum() / weight.sum()
