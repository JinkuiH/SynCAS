...
class Unet(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False, CL=False):
        if len(layers) == 0:
            """Standard forward"""
            # CL_id = [19, 26,40,47,54,64]
            CL_id = [26,47,54,64]
            enc_feats = []
            CL_feat = []
            feat = input
...
class ProjectionHead(nn.Module):
    ...
    def forward(self, x):
        # x: [B, C, D, H, W] → flatten and apply MLP
        B, C, D, H, W = x.shape
        x = x.view(B, C, -1).permute(0, 2, 1)  # [B, N, C]
        x = self.mlp(x)                        # [B, N, out_dim]
        x = x.permute(0, 2, 1).view(B, -1, D, H, W)  # [B, out_dim, D, H, W]
...
#             nn.ReLU(inplace=True),
#             nn.Linear(hidden_dim, out_dim),
#             nn.BatchNorm1d(out_dim)  # 可选，MoCo v2 不加这一层
#         )
...
#         x = x.view(B, -1, D, H, W)             # reshape: [B, out_dim, D, H, W]
#         return x